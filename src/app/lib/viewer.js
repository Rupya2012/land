...
// Using local files instead of CDN for better performance
const DRACO_LOADER = new DRACOLoader(MANAGER).setDecoderPath('/draco/');
const KTX2_LOADER = new KTX2Loader(MANAGER).setTranscoderPath('/basis/');
...
export class Viewer {
	constructor(el, options) {
		...
		// Remove stats for clean display
		...
		this.scene.background = null; // Transparent background for starfield
		...
		// Remove orbit controls for clean display
		this.controls = null;
		...
		// Remove GUI and axes helper for clean display
		// this.addAxesHelper();
		// this.addGUI().then(() => {
	...
	animate(time) {
		...
		// Remove controls and stats updates for clean display
	...
	render() {
		...
		// Remove axes rendering for clean display
		// if (this.state.grid) {
	...
	resize() {
		...
		// Remove axes camera resize for clean display
	...
	setContent(object, clips) {
		...
		// Remove controls reset for clean display
		// this.controls.reset();
		...
		// Remove controls max distance for clean display
		...
		// Remove axes camera setup for clean display
		// this.axesCamera.position.copy(this.defaultCamera.position);
		// this.axesCamera.lookAt(this.axesScene.position);
		...
		// Remove controls save state for clean display
		...
		this.updateLights();
		// Remove GUI update for clean display
		// this.updateGUI();
	...
	setCamera(name) {
		if (name === DEFAULT_CAMERA) {
			// Remove controls enable for clean display
			// this.controls.enabled = true;
			this.activeCamera = this.defaultCamera;
		} else {
			// Remove controls disable for clean display
	...
	updateEnvironment() {
		...
		this.getCubeMapTexture(environment).then(({ envMap }) => {
			...
			this.scene.background = null; // Always transparent for starfield
		});
	}
	...
	updateDisplay() {
		...
		if (this.state.grid !== Boolean(this.gridHelper)) {
			if (this.state.grid) {
				...
				this.gridHelper = null;
				this.axesHelper = null;
				// Remove axes renderer clear for clean display
				// this.axesRenderer.clear();
			}
		}

		// Remove auto-rotate controls for clean display
